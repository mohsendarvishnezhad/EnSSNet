...
MP-EnSSNet-RF with Backward Feature Selection:
The Proposed Reverse-Ranked Feature Elimination with Adaptive Reintroduction Feature Selection Method
...
data = scipy.io.loadmat('C:/Users/ACER/Desktop/EnSSNet/Data Set/SanFransisco_4/SanFransisco_4.mat')
gtm = scipy.io.loadmat('C:/Users/ACER/Desktop/EnSSNet/Data Set/SanFransisco_4/GTM.mat')


PolSAR = data['SanFransisco_4']  # PolSAR image data
gt = gtm['GTM']  # GTM
...
# --------------------------
def extract_patch(PolSAR, center, patch_size=32):
...
# --------------------------
# 8. Define the EnSSNet Model Based on EfficientNet-B0
# --------------------------


# Build sparse normalized adjacency for whole batch

def build_batch_adj(features, k=13):
    """
    features: Tensor of shape (B, C, M, M)
    returns: A_norm sparse tensor of shape (N, N) where N = B*M*M
    """
    B, C, M, _ = features.shape
    N = B * M * M
    device = features.device

    # 1. reshape to (C, B*M*M)
    X = features.reshape(B, C, M*M).transpose(0, 2).reshape(N, C)  # (N, C)
    ...
    # 4. build sparse adjacency (COO)
    rows = torch.arange(N, device=device).unsqueeze(1).repeat(1, k).flatten()  # (N*k,)
    cols = knn.flatten()                                                       # (N*k,)
    vals = torch.ones_like(rows, dtype=torch.float, device=device)
    ...
    # 5. symmetric normalize: D^{-1/2} A D^{-1/2}
    deg = torch.sparse.sum(A, dim=1).to_dense()              # (N,)
    deg_inv_sqrt = deg.pow(-0.5)
    deg_inv_sqrt[deg_inv_sqrt == float('inf')] = 0
    idx = A.indices()
    norm_vals = A.values() * deg_inv_sqrt[idx[0]] * deg_inv_sqrt[idx[1]]
    A_norm = torch.sparse_coo_tensor(idx, norm_vals, size=(N, N)).coalesce()

    return A_norm

#  Custom multi-patch GCN layer

class MultiPatchGCN(nn.Module):
    def __init__(self, in_channels, out_channels, patch_size, k_neighbors=13):
        super().__init__()
        self.C = in_channels   # C
        self.K = out_channels  # K
        self.M = patch_size    # M
        self.k = k_neighbors
        # conv1x1 for per-pixel feature
    ...
    def forward(self, features):
        ...
        # 1. Build A_norm once for whole batch
        A_norm = build_batch_adj(features, k=self.k)  # sparse (N,N)

        # 2. Flatten features to (N, C)
        X = features.reshape(B, C, M*M).transpose(0, 2).reshape(N, C)  # (N,C)

        # 3. Graph convolution: Z = A_norm @ X → (N, C)
        Z = torch.sparse.mm(A_norm, X)  # (N, C)

        # 4. reshape back to (B, C, M, M)
        Z = Z.reshape(B, M*M, C).transpose(1, 2).reshape(B, C, M, M)

        # 5. per-patch per-pixel 1×1 conv + ReLU + avg-pool → (B, K)
        out = self.conv1x1(Z)      # (B, K, M, M)
        out = F.relu(out)
        out = self.pool(out).view(B, self.K)  # (B, K)

        return out

# Full SS Model with EfficientNet-B0 + MultiPatchGCN

class EnSSNetWithGCNModel(nn.Module):
    def __init__(self, in_channels, gcn_out_channels, patch_size, k_neighbors=13):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # 2. Multi-patch GCN path
        fg = self.gcn(x)    # (B, gcn_out_channels)

        # 3. Concatenate
        return torch.cat([fe, fg], dim=1)
...
def train_with_cumulative_curriculum(patches, difficulties, batch_size, model, optimizer, device, epoch, total_epochs):
    ...
    for iteration, batch_idx in enumerate(sorted_batch_indices, 1):
        ...
        # Log progress
        print(f"Epoch [{epoch}/{total_epochs}] Iteration [{iteration}/{len(batches)}] - Trained on {len(cumulative_indices)} samples")
...
# --------------------------
def extract_features(model, PolSAR, patch_size=32, centers=None, device='cuda'):
    """
...
def get_labeled_centers(gt, samples_per_class=5, num_classes=4, seed=42):
...
# --------------------------------
# 15. Feature Selection : BackWared
# --------------------------------

def reverse_rollback_feature_selection_verbose(X_train, y_train, X_test, y_test):

    """
    BackWard feature selection with rollback.
    ...
    # Sort features ascending by MI for backward elimination
    mi = mutual_info_classif(X_test, y_int, discrete_features=False)
    ranked = list(np.argsort(mi))  # # ascending order of MI (remove least important first)
    print("Ranking features by MI(feature, test_pred):")
    for idx in ranked:
        print(f"  F{idx}: MI = {mi[idx]:.4f}")
    print("Feature ranking (desc):", [f"F{i}" for i in ranked], "\n")


    # Initialize full feature set S and removed list R
    S = ranked.copy()  # features from least to most important
    model = SVC(kernel='rbf')
    model.fit(X_train[:, S], y_train)
    y_pred = model.predict(X_test[:, S])
    acc = accuracy_score(y_test, y_pred)
    R = []  # removed features
    print(f"Initial full set: {[f'F{i}' for i in S]} => accuracy {acc:.4f}\n")

    # Backward elimination with rollback
    for k, feat in enumerate(ranked, start=1):
        print(f"---\nStep {k}: Attempt to remove least important feature F{feat}")
        if feat not in S:
            print(f"  F{feat} already removed, skipping.\n")
            continue
        # Tentative removal
        S_temp = [f for f in S if f != feat]
        model.fit(X_train[:, S_temp], y_train)
        y_pred = model.predict(X_test[:, S_temp])
        acc_temp = accuracy_score(y_test, y_pred)
        print(f"  After removal of F{feat}: {[f'F{i}' for i in S_temp]} => accuracy {acc_temp:.4f}")
        ...
        if acc_temp >= acc:
            ...
            acc = acc_temp
            R.append(feat)
            print(f"  Removed F{feat}. New set: {[f'F{i}' for i in S]} => accuracy {acc:.4f}\n")
        else:
            # Rollback phase: try re-adding removed features
            print(f"  Accuracy decreased. Entering Rollback phase to re-add features.\n")
            improved = True
            while improved:
                improved = False
                # sort removed by descending MI (re-add most important removed first)
                for r in sorted(R, key=lambda i: mi[i], reverse=True):
                    S_next = S + [r]
                    model.fit(X_train[:, S_next], y_train)
                    y_pred = model.predict(X_test[:, S_next])
                    acc_next = accuracy_score(y_test, y_pred)
                    print(f"    Try re-adding F{r} -> {[f'F{i}' for i in S_next]} => accuracy {acc_next:.4f}")
                    if acc_next > acc:
                        S = S_next
                        acc = acc_next
                        R.remove(r)
                        improved = True
                        print(f"    Re-added F{r}. Updated set: {[f'F{i}' for i in S]} => accuracy {acc:.4f}\n")
                        break
            print(f"  Rollback complete. Current set: {[f'F{i}' for i in S]} => accuracy {acc:.4f}\n")

    print(f"Final selected features: {[f'F{i}' for i in S]} => accuracy {acc:.4f}")
    return S, acc
...
def main():
    ...
    K, M, _ = view1.shape
    model = EnSSNetWithGCNModel(gcn_in_channels=3,gcn_out_channels=128,patch_size= M,k_neighbors=13).to(device)
    ...
    for band_idx in range(num_bands):
        ...
        # Combine features from different patch sizes and Polarimetric features
        ...
        # Feature Selection Uising The Proposed Feature Fowrwark Selection
        x_train = combined_features
        x_test = combined_all_features
        y_train = labels
        true_labels = np.array([gt[i, j] for (i, j) in all_centers])
        y_test = true_labels
        selected_feats, log_history = reverse_rollback_feature_selection_verbose(x_train, y_train, x_test, y_test)
        x_train_selected = x_train[:, selected_feats]
        x_test_selected = x_test[:, selected_feats]


        # Apply Local Graph-Based Fusion on the extracted feature
        ...
        # Train the RF Classifier using the fused features
        start_time = time.time()
        clf = RandomForestClassifier(n_estimators=500, random_state=42)
        clf.fit(x_train_selected, labels)
        end_time = time.time()
        minutes = int((end_time - start_time) // 60)
        seconds = int((end_time - start_time) % 60)
        print(f"RF Classifier Has Been Trained For Group {band_idx + 1} in {minutes} minutes and {seconds} seconds")

        probs = clf.predict_proba(x_test_selected)
        print(f"Test Phase For Group {band_idx + 1} Has Been Completed Using The RF Classifier")
