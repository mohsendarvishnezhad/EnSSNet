...
def build_batch_adj(features, k=49):
    """
...
class MultiPatchGCN(nn.Module):
    def __init__(self, in_channels, out_channels, patch_size, k_neighbors=49):
        super().__init__()
...
class EnSSNetWithGCNModel(nn.Module):
    def __init__(self, in_channels, gcn_out_channels, patch_size, k_neighbors=49):
        super().__init__()
...
def reverse_rollback_feature_selection_verbose(X_train, y_train, X_test, y_test):
    ...
    First filters features with MI ≥ 40% of max MI (w.r.t. pseudo-labels),
    then performs backward elimination with rollback.
    """
    ...
    # Step 1: Compute MI between each feature and pseudo-labels
    ...
    if len(selected_candidates) == 0:
        selected_candidates = [np.argmax(mi)] 
    ...
    print(f"Selected {len(selected_candidates)} features with MI ≥ 40% of max MI ({mi_max:.4f}):")
    for i in selected_candidates:
        print(f"  F{i}: MI = {mi[i]:.4f}")
    print()

    # Step 2: Rank only the selected features (ascending by MI)
    ranked = sorted(selected_candidates, key=lambda i: mi[i])  # least to most important

    # Initialize full set S with selected and ranked features
    S = ranked.copy()
    ...
    # Step 3: Backward elimination with rollback
...
def main():
    ...
    K, M, _ = view1.shape
    model = EnSSNetWithGCNModel(in_channels=3,gcn_out_channels=128,patch_size= M,k_neighbors=49).to(device)
